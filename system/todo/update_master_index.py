...
def scan_repository_directories(repo_root):
    ...
    for item in repo_root.iterdir():
        if (item.is_dir() and 
            ...
            status = "active" if index_file and Path(repo_root / index_file).exists() else "inactive"
...
def update_master_index(master_index, found_areas, repo_root):
    ...
    for area in master_index["areas"]:
        ...
        if key in found_areas:
            ...
            if updated_area["dir"] != found_area["dir"]:
                print(f"Updating directory path for '{key}': {updated_area['dir']} -> {found_area['dir']}")
                updated_area["dir"] = found_area["dir"]
            
            if updated_area["index"] != found_area["index"]:
                print(f"Updating index path for '{key}': {updated_area['index']} -> {found_area['index']}")
                updated_area["index"] = found_area["index"]
            
            # Update status based on current state
            if updated_area["status"] != found_area["status"]:
                print(f"Updating status for '{key}': {updated_area['status']} -> {found_area['status']}")
    ...
    # Update metadata
    master_index["meta"]["last_modified"] = datetime.now().isoformat() + "+00:00"
